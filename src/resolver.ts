...
const standaloneUrls = (): string[] => STANDALONE_PORTS.map((p) => `http://localhost:${p}/resolve`);
...
function loadEndpoint(): string | null {
  try {
...
function rememberEndpoint(url: string | null) {
  try {
    if (typeof window === "undefined") return;
    if (url) window.sessionStorage.setItem(ENDPOINT_STORAGE_KEY, url);
    else window.sessionStorage.removeItem(ENDPOINT_STORAGE_KEY);
  } catch {
    /* sessionStorage unavailable (SSR, privacy mode) — fall back to in-memory */
  }
}

let detectedEndpoint: string | null = loadEndpoint();
...
const BACKEND_COOLDOWN_MS = 10_000;
...
async function request(body: Record<string, unknown>, override?: string): Promise<ResolveResponse> {
  ...
  if (Date.now() < noBackendUntil) return {};
