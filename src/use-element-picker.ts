...
import type { DevToolsOptions, PickerState, SourceCandidate as PublicSourceCandidate } from "./types";
...
function formatDebugSource(src: DebugSource): string {
  ...
  if (bestIdx > 0) file = file.slice(bestIdx + 1);
 ...
 * occurrence only.
 */
function isLikelyProjectFile(file: string): boolean {
  if (!file) return false;
  if (file.includes("/node_modules/")) return false;
  if (file.startsWith("node:")) return false;
  if (file.includes("react-dom")) return false;
  if (file.includes("next/dist/")) return false;
  return true;
}
...
function scoreSource(name: string, file: string, currentRoute: string): number {
  let score = 0;
  const lower = file.toLowerCase();

  if (lower.includes("/app/") || lower.includes("/pages/")) score += 20;
  if (lower.includes("/features/") || lower.includes("/screens/") || lower.includes("/sections/")) score += 12;
  if (currentRoute) {
    const firstSeg = currentRoute.split("/").filter(Boolean)[0];
    if (firstSeg && lower.includes(`/${firstSeg}/`)) score += 8;
  }
  if (/(?:Page|Screen|Section|View)\.[tj]sx?$/i.test(file)) score += 6;

  const basename = file.split("/").pop()?.replace(/\.[tj]sx?$/, "") || "";
  if (basename && basename === name) score += 2;

  if (lower.includes("/components/ui/") || lower.includes("/components/primitives/") || /\/ui\//.test(lower)) score -= 15;
  if (lower.includes("/node_modules/")) score -= 10;

  if (ATOMIC_NAMES.has(name)) score -= 5;
...
function collectSourceCandidates(
  ...
  const pushCandidate = (name: string, src: DebugSource | null | undefined) => {
    if (!src) return;
    const file = formatDebugSource(src);
    if (!file || !isLikelyProjectFile(file)) return;
    const key = `${name}|${file}`;
  ...
  try {
    ...
    // surface both.
    if (fiber?._debugSource) pushCandidate("(clicked)", fiber._debugSource);
...
function isSemanticAnchor(el: HTMLElement): boolean {
  return (
    !!el.id ||
