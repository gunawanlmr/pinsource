...
type ToBlobFn = (node: HTMLElement, options?: Record<string, unknown>) => Promise<Blob | null>;

function toMessage(err: unknown, fallback: string): string {
  if (err instanceof Error && err.message) return err.message;
  if (typeof err === "string" && err) return err;
  if (err && typeof err === "object") {
    const anyErr = err as { message?: string; type?: string; target?: { src?: string } };
...
async function copyBlobToClipboard(blob: Blob): Promise<{ ok: true } | { ok: false; reason: string }> {
 ...
 */
function baseFilter(node: Node): boolean {
...
function stripMediaFilter(node: Node): boolean {
  if (!baseFilter(node)) return false;
  if (!(node instanceof HTMLElement)) return true;
  const tag = node.tagName;
  if (tag === "IMG" || tag === "VIDEO" || tag === "IFRAME" || tag === "CANVAS") return false;
  return true;
}
...
async function runAttempt(
  ...
  try {
    ...
    if (!blob) return { blob: null, error: "Empty blob (likely canvas taint)", attempt };
...
export async function captureElement(
  ...
  const warnBuffer: string[] = [];
  console.warn = (...args: unknown[]) => {
    const msg = args.map((a) => (typeof a === "string" ? a : "")).join(" ");
    if (msg.startsWith("Failed to fetch resource")) warnBuffer.push(msg);
    else originalWarn.apply(console, args as Parameters<typeof console.warn>);
  };
  ...
  try {
    ...
    attempts.push(
      await runAttempt(toBlob, el, "full", {
        cacheBust: true,
        pixelRatio,
        filter: baseFilter,
      }),
    );
    if (attempts[0].blob) return finish(attempts[0].blob);

    // Attempt 2: skip fonts (font fetches often fail on locked-down CSPs),
    // replace broken images with a transparent 1x1.
    attempts.push(
      await runAttempt(toBlob, el, "skip-fonts", {
        cacheBust: true,
        pixelRatio,
        skipFonts: true,
    ...
    if (attempts[1].blob) return finish(attempts[1].blob);
    ...
    attempts.push(
      await runAttempt(toBlob, el, "no-media", {
        cacheBust: true,
        pixelRatio,
        skipFonts: true,
    ...
    if (attempts[2].blob) return finish(attempts[2].blob);
